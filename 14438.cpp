...
void update(vector<int> &seg, int node, int from, int to, int index, int val){
    if (index < from || to < index) return;
    if (from == to) {
        seg[node] = val;
        return;
    }
    update(seg, node*2, from, (from+to)/2, index, val);
    update(seg, node*2+1, (from+to)/2+1, to, index, val);
    seg[node] = min(seg[node*2], seg[node*2+1]);
}
...
int main(){
    ...
    while(M--){
        cin >> p >> a >> b;
        if(p==1) update(seg, 1, 0, N-1, a-1, b);
        else cout << query(seg, 1, 0, N-1, a-1, b-1) << '\n';
    }
}